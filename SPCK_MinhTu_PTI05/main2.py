...
# from show_data import some_function
# from common import DK, accDatabase
...
def taixiu_w():
    global ui
    ui = taixiu.Ui_MainWindow()  # Đảm bảo đúng lớp Ui_MainWindow từ file taixiu.py
    ui.setupUi(MainWindow)
    ui.btn_draw.clicked.connect(player_draw)  # Kết nối sự kiện
    ui.btn_stop.clicked.connect(player_stand)  # Kết nối sự kiện
    ui.btn_reset.clicked.connect(reset_game)  # Kết nối sự kiện
    MainWindow.show()
...
def update_display():
    ui.card_display.setPlainText("\n".join(pl1))
    ui.score_label.setText(f"Điểm: {scpl1}")
...
def login_check():
    ...
    check = acc_db.check_login(username, password)
    
    if check:
        msg_box('Login success', 'Welcome to my app !!!')
        gameUi()  # Mở màn hình game sau khi đăng nhập thành công
    else:
        msg_box('Login fail', 'Username or password is incorrect!')  # Thông báo lỗi nếu đăng nhập thất bại
...
def signup_check():
    ...
    if not all([name, username, password, age]):
    ...
    acc_db.add_acc(new_user)
    msg_box('Đăng ký thành công', 'Tài khoản đã được tạo.')
    gameUi()
